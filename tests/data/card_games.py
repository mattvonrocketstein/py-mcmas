# Porting card_games.ispl to a raw python datastructure
model_dict_raw = dict(
    environment=dict(
        obsvars=dict(win="boolean"),
        vars=dict(cards="{null, ak, aq, ka, kq, qa, qk};"),
        actions="none".split(),
    ...
    evaluation=["p1win if Environment.win=true;"],
    init_states=[
        "(player1.step=s1) and (Environment.cards=null) and Environment.win=false;"
    ],
    groups=dict(g1=["player1"]),
    formulae=["AF(p1win);", "<g1>F(p1win);"],
)

model_dict = model_dict_raw
